...
for k = 1:1 % nicht length(T) - nicht für alle Verbindungen, sondern nur 
            % für die von einem Knotenpunkt
    t     = T(k);
    % [d,p] = FstSearch(E,n,t);
    [d,p] = BFS(E,n,t);

    if any(d(J)~=inf), % wenn erreichbar
        j = J(d(J)==min(d(J)));
        text(P(t,1)+xofs,P(t,2),['\leftarrow ',int2str(d(j))],'verticalalignment','middle');
        %
        while j~=t,
            set(EHdl(E==(max(j,p(j))-1)*n+min(j,p(j))),'color','r'); % markiere kürzeste Verbindung in rot
            j = p(j);
        end;
        %
        drawnow;
